...
import type { VoiceOption, TTSProviderType } from "../../../shared/types";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (provider === "piper") {
          result = await window.ferroAPI.listPiperVoices();
        } else if (provider === "elevenlabs" && apiKey) {
          result = await window.ferroAPI.listElevenLabsVoices(apiKey);
        } else if (provider === "system") {
          result = await window.ferroAPI.listSystemVoices();
        }
        setVoices(result);
      } catch {
        setVoices([]);
